# KronosChamber VS Code Extension
...
KronosChamber inside VS Code: embeds the KronosChamber chat UI in the activity bar and connects it to the [KronosCode](https://kronoscode.ai) API.
...
| Command | Description |
|---------|-------------|
| `KronosChamber: Focus on Chat View` | Focus chat panel |
| `KronosChamber: Restart API Connection` | Restart KronosCode API process |
| `KronosChamber: Show KronosCode Status` | Provide debug info useful for development or bug report |

## Configuration

| Setting | Default | Description |
|---------|---------|-------------|
| `openchamber.apiUrl` | `http://localhost:47339` | KronosCode API server URL. Not required by default. Spawns its own process when not set. |

## Requirements

- KronosCode CLI installed and available in PATH (or set via `KRONOSCODE_BINARY` env var)
- VS Code 1.85.0+
