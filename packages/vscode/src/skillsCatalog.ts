...
import { discoverSkills } from './kronoscodeConfig';
...
type SkillInstallSource = 'kronoscode' | 'agents';
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    ...
    const response = await fetch(url, {
      ...
      headers: {
        ...
        'User-Agent': 'KronosChamber-VSCode/1.0',
        ...options?.headers,
      },
    });
...
export async function installSkillsFromClawdHub(options: {
  ...
  const targetSource: SkillInstallSource = options.targetSource === 'agents' ? 'agents' : 'kronoscode';
  ...
  for (const sel of requestedSkills) {
    ...
    const targetDir = options.scope === 'user'
      ...
      : (targetSource === 'agents'
        ...
        : path.join(options.workingDirectory as string, '.kronoscode', 'skills', slug));
...
function getUserSkillBaseDir() {
  const pluralPath = path.join(os.homedir(), '.config', 'kronoscode', 'skills');
  const legacyPath = path.join(os.homedir(), '.config', 'kronoscode', 'skill');
  ...
  for (const plan of skillPlans) {
    ...
    const targetDir = options.scope === 'user'
      ...
      : (targetSource === 'agents'
        ...
        : path.join(options.workingDirectory as string, '.kronoscode', 'skills', plan.skillName));
...
export async function getSkillsCatalog(
  ...
  installedSkills?: Array<{ name: string; scope: SkillScope; source?: 'kronoscode' | 'agents' | 'claude' }>
) {
    ...
    if (isClawdHubSource(src.source)) {
      ...
      itemsBySource[src.id] = items.map((item) => {
        ...
        return {
          sourceId: src.id,
          ...item,
          installed: installed ? { isInstalled: true, scope: installed.scope, source: installed.source === 'agents' ? 'agents' : 'kronoscode' } : { isInstalled: false },
        };
      });
      continue;
    }
